...
def timePerf(command,n=10):
    ...
    return generateDataFromStringOutput(output) 

def generateDataFromStringOutput(output):
    expr = re.compile( r"Time\t(.*)\t(.*)\t+(\d+)") 
    ms=expr.findall(output)

    labels= [label for label,_,_ in ms ]
    timings=[float(time) for _,time,_ in ms ]
    cores=[float(cpu) for _,_,cpu in ms ]


    data=pd.DataFrame({"label":labels , "time":timings, "cores":cores})
    return data
